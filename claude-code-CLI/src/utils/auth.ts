...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test' || process.env.ANTHROPIC_BASE_URL) {
  ...
  // Allow any API key when using a custom proxy
  if (apiKeyEnv && process.env.ANTHROPIC_BASE_URL) {
    console.error('[DEBUG-AUTH] Returning key via ANTHROPIC_BASE_URL bypass, key=' + apiKeyEnv)
    return {
      key: apiKeyEnv,
      source: 'ANTHROPIC_API_KEY',
    }
  }
  console.error('[DEBUG-AUTH] apiKeyEnv=' + apiKeyEnv + ' BASE_URL=' + process.env.ANTHROPIC_BASE_URL)
  if (
    apiKeyEnv &&

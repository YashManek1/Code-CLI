...
export async function pathExists(path: string): Promise<boolean> {
  process.stderr.write(`[DEBUG] pathExists: checking ${path}...\n`)
  try {
    await stat(path)
    process.stderr.write(`[DEBUG] pathExists: ${path} exists.\n`)
    return true
  } catch {
    process.stderr.write(`[DEBUG] pathExists: ${path} does NOT exist.\n`)
    return false
  }
}

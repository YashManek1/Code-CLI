...
export const getPluginCommands = memoize(async (): Promise<Command[]> => {
  ...
  // Process plugins sequentially to avoid Windows I/O contention
  const perPluginCommands: Command[][] = []
  for (const plugin of enabled) {
    ...
    if (plugin.commandsPaths) {
      ...
      // Process all commandsPaths sequentially
      for (const commandPath of plugin.commandsPaths) {
        try {
          const fs = getFsImplementation()
    ...
    perPluginCommands.push(pluginCommands)
  }
...
export const getPluginSkills = memoize(async (): Promise<Command[]> => {
  ...
  for (const plugin of enabled) {
    ...
    if (plugin.skillsPaths) {
      ...
      // Process all skillsPaths sequentially
    ...
    perPluginSkills.push(pluginSkills)
  }

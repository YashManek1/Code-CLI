...
async function readCachedMarketplace(
  ...
  process.stderr.write(`[DEBUG] readCachedMarketplace: trying nested path ${nestedPath}...\n`)
  try {
    const result = await parseFileWithSchema(nestedPath, PluginMarketplaceSchema())
    process.stderr.write(`[DEBUG] readCachedMarketplace: success with nested path.\n`)
    return result
  } catch (e) {
    if (e instanceof ConfigParseError) {
      process.stderr.write(`[DEBUG] readCachedMarketplace: ConfigParseError for nested path: ${errorMessage(e)}\n`)
      throw e
    }
    const code = getErrnoCode(e)
    if (code !== 'ENOENT' && code !== 'ENOTDIR') {
      process.stderr.write(`[DEBUG] readCachedMarketplace: error for nested path: ${errorMessage(e)}\n`)
      throw e
    }
  }
  process.stderr.write(`[DEBUG] readCachedMarketplace: nested path failed, trying direct path ${installLocation}...\n`)
  const result = await parseFileWithSchema(installLocation, PluginMarketplaceSchema())
  process.stderr.write(`[DEBUG] readCachedMarketplace: success with direct path.\n`)
  return result
}
...
export async function getMarketplaceCacheOnly(
  ...
  process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: starting for ${name}...\n`)
  const fs = getFsImplementation()
  const configFile = getKnownMarketplacesFile()

  try {
    process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: reading configFile ${configFile}...\n`)
    const content = await fs.readFile(configFile, { encoding: 'utf-8' })
    const config = jsonParse(content) as KnownMarketplacesConfig
    const entry = config[name]

    if (!entry) {
      process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: no entry for ${name}.\n`)
      return null
    }

    process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: found entry for ${name}, installLocation=${entry.installLocation}. Calling readCachedMarketplace...\n`)
    return await readCachedMarketplace(entry.installLocation)
  } catch (error) {
    if (isENOENT(error)) {
      process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: ENOENT for ${name}.\n`)
      return null
    }
    process.stderr.write(`[DEBUG] getMarketplaceCacheOnly: error for ${name}: ${errorMessage(error)}\n`)
    logForDebugging(

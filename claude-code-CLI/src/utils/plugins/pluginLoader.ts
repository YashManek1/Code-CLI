...
export async function installFromGitSubdir(
  ...
  try {
    ...
    if (sha) {
      ...
      const revParse = await execFileNoThrowWithCwd(
        gitExe(),
        ['rev-parse', 'HEAD'],
        {
          cwd: cloneDir,
...
async function validatePluginPaths(
  ...
  // Process paths sequentially to avoid Windows I/O contention
...
export async function createPluginFromPath(
  ...
  const commandsDirExists = !manifest.commands
    ? await pathExists(join(pluginPath, 'commands'))
    : false
  const agentsDirExists = !manifest.agents
    ? await pathExists(join(pluginPath, 'agents'))
    : false
  const skillsDirExists = !manifest.skills
    ? await pathExists(join(pluginPath, 'skills'))
    : false
  const outputStylesDirExists = !manifest.outputStyles
    ? await pathExists(join(pluginPath, 'output-styles'))
    : false
  ...
  if (manifest.commands) {
    ...
    if (
      ...
      // Process each command entry sequentially
      for (const [commandName, metadata] of Object.entries(manifest.commands)) {
        ...
        if (metadata.source) {
          ...
          if (await pathExists(fullPath)) {
            validPaths.push(fullPath)
            commandsMetadata[commandName] = metadata
          } else {
            logForDebugging(
              `Command ${commandName} path ${metadata.source} specified in manifest but not found at ${fullPath} for ${manifest.name}`,
              { level: 'warn' },
            )
            logError(
              new Error(
                `Plugin component file not found: ${fullPath} for ${manifest.name}`,
              ),
            )
            errors.push({
              type: 'path-not-found',
      ...
      // Process each command path sequentially
      for (const cmdPath of commandPaths) {
        ...
        if (await pathExists(fullPath)) {
          ...
          logForDebugging(
            `Command path ${cmdPath} specified in manifest but not found at ${fullPath} for ${manifest.name}`,
            { level: 'warn' },
          )
          logError(
            new Error(
...
async function loadPluginSettings(
  ...
  if (await pathExists(settingsJsonPath)) {
    try {
      ...
      // Failed to parse or read despite existing
      logForDebugging(
  ...
  for (const [event, matchers] of Object.entries(additional)) {
    if (!merged[event as keyof HooksSettings]) {
      ...
      merged[event as keyof HooksSettings] = [
        ...((merged[event as keyof HooksSettings] as unknown[]) || []),
        ...(matchers as unknown[]),
      ] as typeof matchers
    }
  }
...
async function loadPluginsFromMarketplaces({
  ...
  logForDebugging(`loadPluginsFromMarketplaces: starting (cacheOnly=${cacheOnly})...`)
  ...
  >()
  logForDebugging(`loadPluginsFromMarketplaces: loading ${uniqueMarketplaces.size} unique marketplaces: ${[...uniqueMarketplaces].join(', ')}`)
  for (const name of uniqueMarketplaces) {
    logForDebugging(`loadPluginsFromMarketplaces: loading marketplace catalog for ${name}...`)
    const catalog = await getMarketplaceCacheOnly(name)
    marketplaceCatalogs.set(name, catalog)
    logForDebugging(`loadPluginsFromMarketplaces: loaded marketplace catalog for ${name}.`)
  }
  logForDebugging('loadPluginsFromMarketplaces: all unique marketplace catalogs loaded.')
  ...
  // Load all marketplace plugins sequentially to avoid Windows I/O contention
  const results: PromiseSettledResult<LoadedPlugin | null>[] = []
  for (const [pluginId, enabledValue] of marketplacePluginEntries) {
    try {
      ...
      const loadedPlugin = await (cacheOnly
      ...
      results.push({ status: 'fulfilled', value: loadedPlugin })
    } catch (e) {
  ...
  if (!hasManifest) {
    ...
    if (entry.commands) {
      ...
      if (
        ...
        const checks = []
        for (const [commandName, metadata] of entries) {
          if (!metadata || typeof metadata !== 'object' || !metadata.source) {
            checks.push({ commandName, metadata, skip: true as const })
            continue
          }
          const fullPath = join(pluginPath, metadata.source)
          checks.push({
        ...
        for (const cmdPath of commandPaths) {
          if (typeof cmdPath !== 'string') {
            checks.push({ cmdPath, kind: 'invalid' as const })
            continue
          }
          const fullPath = join(pluginPath, cmdPath)
          checks.push({
    ...
    if (entry.skills) {
      ...
      for (const skillPath of skillPaths) {
        ...
        checks.push({ skillPath, fullPath, exists: await pathExists(fullPath) })
      }
      const validPaths: string[] = []
...
async function assemblePluginLoadResult(
  ...
  const marketplaceResult = await marketplaceLoader()
  const sessionResult =
    inlinePlugins.length > 0
      ? await loadSessionOnlyPlugins(inlinePlugins)
      : { plugins: [], errors: [] }

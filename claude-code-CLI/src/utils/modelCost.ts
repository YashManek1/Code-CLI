...
function normalizeUsage(usage?: Usage): Usage {
  ...
  return {
    ...EMPTY_USAGE,
    ...usage,
    server_tool_use: usage.server_tool_use ?? EMPTY_USAGE.server_tool_use,
  } as Usage;
}
...
export function getModelCosts(model: string, usage?: Usage): ModelCosts {
  ...
  if (
    ...
    const isFastMode = safeUsage.speed === "fast";
...
export function calculateUSDCost(resolvedModel: string, usage?: Usage): number {
  const safeUsage = normalizeUsage(usage);
  const modelCosts = getModelCosts(resolvedModel, safeUsage);
  return tokensToUSDCost(modelCosts, safeUsage);
}

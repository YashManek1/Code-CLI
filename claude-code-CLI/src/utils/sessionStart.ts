...
export async function processSessionStartHooks(
  ...
  if (shouldAllowManagedHooksOnly()) {
    ...
    try {
      logForDebugging(`processSessionStartHooks: loading plugin hooks for ${source}...`)
      await withDiagnosticsTiming('load_plugin_hooks', () => loadPluginHooks())
      logForDebugging(`processSessionStartHooks: plugin hooks loaded for ${source}.`)
    } catch (error) {
      logForDebugging(`processSessionStartHooks: failed to load plugin hooks: ${error}`)

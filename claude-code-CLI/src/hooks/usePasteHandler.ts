...
export function usePasteHandler({
  ...
  const supportsEmptyPasteImage = isMacOS || isWindows;
      ...
      return setTimeout(
        (
          ...
          setPasteState(({ chunks }) => {
            ...
            // If paste is empty (common when trying to paste images with Cmd+V on macOS
            // or Ctrl+V in Windows Terminal), check if clipboard has an image.
            if (
              supportsEmptyPasteImage &&
              onImagePaste &&
              pastedText.length === 0
            ) {
              checkClipboardForImage();

...
export function addToTotalSessionCost(
  ...
  const safeUsage = usage
    ? ({ ...EMPTY_USAGE, ...usage } as Usage)
    : (EMPTY_USAGE as Usage);
  ...
  if (tokenCounter) {
    tokenCounter.add(safeUsage.input_tokens, { ...attrs, type: "input" });
    tokenCounter.add(safeUsage.output_tokens, { ...attrs, type: "output" });
    tokenCounter.add(safeUsage.cache_read_input_tokens ?? 0, {
      ...attrs,
      type: "cacheRead",
    });
    tokenCounter.add(safeUsage.cache_creation_input_tokens ?? 0, {
      ...attrs,
      type: "cacheCreation",
    });
  }
